/* Based on huff.c by Moodstocks SAS */

#include "mSpinJPEGenc.h"
#include <limits.h>	// required for WORD_BIT, CHAR_BIT, etc.
#include <string.h> // for memset in jpec_huff_skel_init
...
jpec_huff_t *jpec_huff_new(void) {
  jpec_huff_t *h = sark_alloc(1, sizeof(*h));
  h->state.buffer = 0;
  h->state.nbits = 0;
  h->state.dc = 0;
  h->state.buf = NULL;
...
void jpec_huff_encode_block_impl(jpec_block_t *block, jpec_huff_state_t *s) {
  ...
  if (nbits) jpec_huff_write_bits(s, (uint) bits, nbits);
